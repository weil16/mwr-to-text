...
def test_data_loading(load_and_preprocess):
...
def test_model_initialization(MWRModel, get_lora_config):
...
def test_training_workflow(MWRDataset, MWRModel, get_lora_config, load_and_preprocess):
    ...
    try:
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        tokenizer = T5Tokenizer.from_pretrained('t5-small', legacy=True)  # Explicitly set legacy=True to maintain backward compatibility
...
def test_evaluate_function(evaluate):
...
def test_config_validation():
    ...
    try:
        ...
        required_keys = [
            ...
            'training.log_dir', 'training.inference_log_dir'
        ]
...
def run_full_test_suite(MWRDataset, MWRModel, get_lora_config, evaluate, load_and_preprocess):
    """Run complete test suite"""
    print("\n=== Running Full Test Suite ===")
    
    tests = [
        lambda: test_data_loading(load_and_preprocess),
        lambda: test_model_initialization(MWRModel, get_lora_config),
        lambda: test_training_workflow(MWRDataset, MWRModel, get_lora_config, load_and_preprocess),
        test_evaluation_metrics,
        lambda: test_evaluate_function(evaluate),
