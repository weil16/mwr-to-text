...
def run_inference(checkpoint_path, config, MWRDataset, MWRModel, get_lora_config, evaluate, load_and_preprocess, dataset_name='test'):
    ...
    if results['bertscore']:
        log_message += f", BERTScore P: {results['bertscore']['precision']:.4f}"
        log_message += f", R: {results['bertscore']['recall']:.4f}"
        log_message += f", F1: {results['bertscore']['f1']:.4f}"
    print(log_message)
